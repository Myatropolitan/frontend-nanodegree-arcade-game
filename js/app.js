...
//(since setting to rowHeight does not look pretty)
var enemySepY = 70;
...
var initialPlayerX = columnWidth * 2;
...
var Enemy = function() {
    ...
    this.x = getRandomInt((columnWidth * -1), 404);
    //set start position y on one of the three rows of stone
    this.y = getRandomInt(1,3) * enemySepY;
...
Enemy.prototype.update = function(dt) {
    ...
    if(nextLoc > canvasWidth){
        this.x = (columnWidth * -1);
        this.y = getRandomInt(1,3) * enemySepY;
        this.speed = getRandomInt(speedMin, speedMax);
        this.ignoreCollision = false;
    ...
    if(player.x < (this.x + columnWidth/2) &&
        player.x + columnWidth/2 > this.x &&
        player.y < (this.y + rowHeight/2) &&
        (player.y + rowHeight/2) > this.y){
...
Player.prototype.update = function() {
    ...
    if(this.x < (heart.x + columnWidth/2) &&
        this.x + columnWidth/2 > heart.x &&
        this.y < (heart.y + rowHeight/2) &&
        (this.y + rowHeight/2) > heart.y){
        this.addLife();
    }
}
...
Player.prototype.handleInput = function(key) {
    ...
    if(key == 'left'){
        newX = this.x - columnWidth;
        //move left if player does not go off canvas
        if(newX >= 0)
            this.x = newX;
    }else if(key == 'up'){
        newY = this.y - rowHeight;
...
Heart.prototype.resetHeart = function() {
    //reset location
    this.x = getRandomInt(0,4) * columnWidth;
    this.y = 320;
}
